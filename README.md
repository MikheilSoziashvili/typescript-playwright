...
│   │   ├── base-page.ts                          # Base page class with generic type support
│   │   ├── base-map.ts                           # Base map interface
│   │   ├── base-asserter.ts                      # Base asserter class
│   │   └── base-steps.ts                         # Base steps class
│   ├── visual-automation                         # Visual automation framework for canvas-based games
│   │   ├── base-visual-page.ts                   # Core visual automation logic with image matching
│   │   ├── base-visual-map.ts                    # Base visual locators interface
│   │   ├── base-visual-steps.ts                  # Visual step wrappers
│   │   ├── base-visual-asserter.ts               # Visual assertions
│   │   ├── [game-name]-page.ts                   # Game-specific visual page (example: zulu-gold-page.ts)
│   │   ├── [game-name]-map.ts                    # Game-specific visual locators
│   │   ├── [game-name]-steps.ts                  # Game-specific visual steps
│   │   └── [game-name]-asserter.ts               # Game-specific visual assertions
...
│   ├── casino-games                              # Casino games page objects
│   │   ├── [provider]                            # Game provider directory (example: bgaming, hacksaw-gaming)
│   │   │   ├── [game-name]-page.ts               # Game-specific page
│   │   │   ├── [game-name]-map.ts                # Game-specific locators
│   │   │   ├── [game-name]-steps.ts              # Game-specific steps
│   │   │   └── [game-name]-asserter.ts           # Game-specific assertions
...
The Visual Automation Framework enables pixel-based image matching for automating canvas-based games and complex UI elements that are difficult to interact with using traditional DOM selectors. This is particularly useful for games rendered on HTML5 canvas elements.

### Key Features

-   **Template Matching** - Uses image matching algorithms to locate visual elements
-   **Flexible Confidence Thresholds** - Adjustable matching confidence levels (STRICT, RELAXED, VERY_RELAXED)
-   **Performance Metrics** - Records timing and success metrics for visual operations
-   **Comprehensive Interactions** - Support for click, double-click, right-click, hover, and drag-drop operations
-   **Assertion Support** - Built-in assertions for verifying visual elements

### Core Components

-   **BaseVisualPage** - Main class handling all visual automation operations and action methods
-   **BaseVisualMap** - Defines locators for visual elements (template image paths)
-   **BaseVisualSteps** - Base class for combining actions and assertions in step methods
-   **BaseVisualAsserter** - Assertion methods for visual elements

### Usage Example

```typescript
// Navigate to game and interact with visual elements
await zuluGoldPage.clickVisualElement("path/to/spin-button.png");
await zuluGoldPage.waitForVisualElement("path/to/win-label.png");
await zuluGoldPage.assertThat().imageFound("path/to/win-label.png");

// Or use steps for combined action + assertion workflows
await zuluGoldPage.steps().spinAndVerifyResult();
```

### Configuration

Visual automation behavior is controlled through enums in `constants/visual-automation.ts`:

-   **VisualComparisonThreshold** - Confidence levels for image matching
-   **VisualAutomationPolling** - Polling intervals and timeouts
-   **VisualAutomationDefaults** - Default color threshold and confidence values

### Best Practices

-   Keep template images small and focused on unique UI elements
-   Adjust confidence thresholds based on element stability
-   Use appropriate polling intervals to balance performance and reliability
-   Implement error handling for optional visual elements
-   Leverage logging for debugging visual operations
