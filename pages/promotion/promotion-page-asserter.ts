...
export class PromotionPageAsserter extends BaseAsserter<PromotionPage> {
	...
	@step("Verify promotion how to participate button text")
	public async promotionHowToParticipateButtonHasText(expectedText: string): Promise<void> {
		await expect(this.gamdomPage.map.promotionHowToParticipateButton).toHaveText(expectedText);
	}
}
