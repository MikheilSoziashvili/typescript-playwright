...
test.describe("Ban user from linking platforms", () => {
    ...
    for (const record of parse_csv(
            ...
            test(`[ENG-1540] UserInfo - Info - Community connect actions: ban linking of ${record.platform}`, async ({
                userInfoAdminPage,
                infoAdminPage,
            }) => {
                ...
                await infoAdminPage
                    .steps()
                    .banUserFromLinkingPlatform(record.platform, record.tableValue);

                await infoAdminPage
                    .steps()
                    .enableUserToLinkPlatform(record.platform, record.tableValue);
            });
    }
});
